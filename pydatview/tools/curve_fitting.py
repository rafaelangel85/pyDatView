...
import six
from numpy import inf  # for user convenience
...
def model_fit(func, x, y, p0=None, bounds=None, **fun_kwargs):
    ...
    if isinstance(func, six.string_types) and func.find('fitter:') == 0:
        ...
        if len(missing) > 0:
            raise Exception(
                'Curve fitting with `{}` requires the following arguments {}. Missing: {}'.format(func, consts.keys(),
                                                                                                  missing))
...
class ModelFitter:
    def __init__(self, func=None, x=None, y=None, p0=None, bounds=None, **fun_kwargs):
        ...
        self.model = {
            ...
            'coeffs': None, 'formula_num': 'unavailable', 'fitted_function': None, 'coeffs_init': p0, 'bounds': bounds,
            # model fitting
            'R2': None,
        }
    ...
    def set_model(self, func, **fun_kwargs):
        if callable(func):
            ...
            if func.find('predef:') == 0:
                ...
                if func not in predef_models:
                    raise Exception(
                        'Predefined function `{}` not defined in curve_fitting module\n Available functions: {}'.format(
                            func, predef_models))
                ...
                if nParams == 0:
                    raise Exception(
                        'Formula should contains parameters in curly brackets, e.g.: {a}, {b}, {u_1}. No parameters found in {}'.format(
                            formula))
                ...
                try:
                    ...
                    if y.shape != x.shape:
                        raise Exception(
                            'The formula does not return an array of same size as the input variable x. The formula must include `x`: {}'.format(
                                formula_eval))
                except SyntaxError:
        ...
        if len(missing) > 0:
            raise Exception('Curve fitting with function `{}` requires the following arguments {}. Missing: {}'.format(
                func.__name__, consts.keys(), missing))
    ...
    def fit_data(self, x, y, p0=None, bounds=None):
        ...
        if p0 is not None:
            ...
            raise Exception(
                'Initial guess `p0` needs to be provided since we cant infer the size of the model coefficients.')
        if self.model['coeffs'] is not None:
            if len(self.model['coeffs']) != nParams:
                raise Exception(
                    'Inconsistent dimension between model guess (size {}) and the model parameters (size {})'.format(
                        nParams, len(self.model['coeffs'])))
...
def extract_key_num(text):
    ...
    return OrderedDict(
        [(match.group("key"), np.float(match.group("value"))) for match in regex.finditer(text.replace(' ', ''))])
...
def extract_key_miscnum(text):
    ...
    for (k, v) in zip(keys, values):
        if v.find('(') >= 0:
            ...
            v = v.replace('[', '').replace(']', '')
            v = v.split(',')
            vect = [int(val) if isint(val) else np.float(val) for val in v if
                    len(val.strip()) > 0]  # NOTE returning lists
        else:
            v = v.replace(',', '').strip()
...
class TestFitting(unittest.TestCase):
    ...
    def test_key_tuples(self):
        ...
        self.assertEqual(extract_key_tuples('a=(1, 2),b =(inf,0),c= ( -inf , 0.3e+10)'),
                         {'a': (1, 2), 'b': (inf, 0), 'c': (-inf, 0.3e+10)})

    def test_key_num(self):
        self.assertEqual(extract_key_num('a=2'), OrderedDict({'a': 2}))
        self.assertEqual(extract_key_num('all=0.1,b =inf, c= -0.3e+10'),
                         OrderedDict({'all': 0.1, 'b': inf, 'c': -0.3e+10}))
