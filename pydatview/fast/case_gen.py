...
def templateReplaceGeneral(PARAMS, templateDir=None, outputDir=None, main_file=None, removeAllowed=False,
                           removeRefSubFiles=False, oneSimPerDir=False):
    ...
    def replaceRecurse(templatename_or_newname, FileKey, ParamKey, ParamValue, Files, strID, workDir, TemplateFiles):
        ...
        if len(ChildrenKeys) == 0:
            ...
            #  
            newchildFilename, Files = replaceRecurse(child_templatefilename, NewFileKey, ChildrenKey, ParamValue, Files,
                                                     strID, workDir, TemplateFiles)
...
def templateReplace(PARAMS, templateDir, outputDir=None, main_file=None, removeAllowed=False, removeRefSubFiles=False,
                    oneSimPerDir=False):
    ...
    return templateReplaceGeneral(PARAMS, templateDir, outputDir=outputDir, main_file=main_file,
                                  removeAllowed=removeAllowed, removeRefSubFiles=removeRefSubFiles,
                                  oneSimPerDir=oneSimPerDir)
...
def paramsWS_RPM_Pitch(WS, RPM, Pitch, baseDict=None, flatInputs=False):
    ...
    for ws, rpm, pitch in zip(WS_flat, RPM_flat, Pitch_flat):
        ...
        p['__index__'] = i
        p['__name__'] = '{:03d}_ws{:04.1f}_pt{:04.2f}_om{:04.2f}'.format(p['__index__'], p['InflowFile|HWindSpeed'],
                                                                         p['EDFile|BlPitch(1)'], p['EDFile|RotSpeed'])
        i = i + 1
        PARAMS.append(p)
    return PARAMS
...
# --------------------------------------------------------------------------------{
def CPCT_LambdaPitch(refdir, main_fastfile, Lambda=None, Pitch=np.linspace(-10, 40, 5), WS=None, Omega=None,
    ...
    fastFiles = templateReplace(PARAMS, refdir, outputDir=workDir, removeRefSubFiles=True, removeAllowed=RemoveAllowed,
    ...
    result = postpro.averagePostPro(outFiles, avgMethod='periods', avgParam=1, ColKeep=ColKeepStats,
