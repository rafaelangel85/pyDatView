...
def insert_radial_columns(df, vr=None, R=None, IR=None):
    ...
    if vr is None or R is None:
        ...
        if nrMax <= len(vr_bar):
            ...
            print(vr_bar)
            raise Exception(
                'Inconsitent length between radial stations ({:d}) and max index present in output chanels ({:d})'.format(
                    len(vr_bar), nrMax))
...
def averageDF(df, avgMethod='periods', avgParam=None, ColMap=None, ColKeep=None, ColSort=None, stats=['mean']):
    ...
    if avgMethod.lower() == 'constantwindow':
        ...
        if 'Azimuth_[deg]' not in df.columns:
            raise Exception(
                'The sensor `Azimuth_[deg]` does not appear to be in the output file. You cannot use the averaging method by `periods`, use `constantwindow` instead.')
        ...
        if avgParam is None:
            ...
            if len(iBef) - 1 < avgParam:
                print('[WARN] Not enough periods found ({}) compared to number requested to average ({})!'.format(
                    len(iBef) - 1, avgParam))
                avgParam = len(iBef) - 1
            if avgParam == 0:
                tStart = time[0]
        ...
        if 'RotSpeed_[rpm]' not in df.columns:
            raise Exception(
                'The sensor `RotSpeed_[rpm]` does not appear to be in the output file. You cannot use the averaging method by `periods_omega`, use `periods` or `constantwindow` instead.')
    ...
    if ColKeep is not None:
        ...
        df = df[ColKeepSafe]
    if tStart < time[0]:
        print('[WARN] Simulation time ({}) too short compared to required averaging window ({})!'.format(tEnd - time[0],
                                                                                                         tStart - tEnd))
...
def averagePostPro(outFiles, avgMethod='periods', avgParam=None, ColMap=None, ColKeep=None, ColSort=None,
                   stats=['mean']):
    ...
    for i, f in enumerate(outFiles):
        try:
            ...
            continue
        postpro = averageDF(df, avgMethod=avgMethod, avgParam=avgParam, ColMap=ColMap, ColKeep=ColKeep, ColSort=ColSort,
                            stats=stats)
