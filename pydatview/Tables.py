...
class Table(object):
    ...
    def __repr__(self):
        return 'Tab {} ({}x{}) (raw: {}, active: {}, file: {})'.format(self.name, self.nCols, self.nRows, self.raw_name,
                                                                       self.active_name, self.filename)
    ...
    def radialAvg(self, avgMethod, avgParam):
        ...
        if sCols.find('WkDf') > 1 or sCols.find('CtT') > 0:
            # --- FAST FARM files
            Files = [base + ext for ext in ['.fstf', '.FSTF', '.Fstf', '.fmas', '.FMAS', '.Fmas'] if
            ...
            if iDotAD > 1:
                base = base[:iDotAD]
            #
            Files = [base + ext for ext in ['.fst', '.FST', '.Fst', '.dvr', '.Dvr', '.DVR'] if
                     os.path.exists(base + ext)]
            if len(Files) == 0:
                fst_in = None
            ...
            dfRadED, dfRadAD, dfRadBD = fastlib.spanwisePostPro(fst_in, avgMethod=avgMethod, avgParam=avgParam,
                                                                out_ext=out_ext, df=self.data)
