...
class PlotData():
    ...
    def __repr__(s):
        s1 = 'id:{}, it:{}, ix:{}, iy:{}, sx:"{}", sy:"{}", st:{}, syl:{}\n'.format(s.id, s.it, s.ix, s.iy, s.sx, s.sy,
                                                                                    s.st, s.syl)
        return s1
    ...
    def toFFT(PD, yType='Amplitude', xType='1/x', avgMethod='Welch', avgWindow='Hamming', bDetrend=True, nExp=8):
        ...
        PD.x, PD.y, Info = fft_wrap(PD.x, PD.y, dt=dt, output_type=yType, averaging=avgMethod,
                                    averaging_window=avgWindow, detrend=bDetrend, nExp=nExp)
    ...
    def _measCalc(PD, mode):
        ...
        try:
            ...
            if mode == 'mean':
                ...
                v = PD.x[
                    left_index + np.where(PD.y[left_index:right_index] == np.nanmin(PD.y[left_index:right_index]))[0][
                        0]]
            elif mode == 'xmax':
                v = PD.x[
                    left_index + np.where(PD.y[left_index:right_index] == np.nanmax(PD.y[left_index:right_index]))[0][
                        0]]
            else:
            ...
            for pd in PD_SameCol[1:]:
                if pd.xIsString:
                    if len(xRef) == len(pd.x):
                        ...
                        raise Exception(
                            'X values have different length and are strings, cannot interpolate string. Use `Index` for x instead.')
                else:
